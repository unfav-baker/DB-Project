...
namespace Dashboard
{
    public partial class Password_b : Page
    {
        ...
        public Password_b(string email)
        {
            ...
            // This requires the button in Password_b.xaml to have x:Name="btnSendResetEmail"
            Button? resetButton = this.FindName("btnSendResetEmail") as Button;

            if (string.IsNullOrEmpty(_connectionString))
            {
                Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] DEBUG: Password_b.xaml.cs - Constructor - Connection string IS NULL or EMPTY. Env Var: '{DbConnectionStringEnvVar}'");
                ...
                {
                    Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] WARNING: Password_b.xaml.cs - Constructor - Could not find button with x:Name='btnSendResetEmail' to disable. Ensure XAML is correct.");
                }
            }
            else
            {
                string connStringSnippet = _connectionString.Length > 30 ? _connectionString.Substring(0, 30) + "..." : _connectionString;
                Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] DEBUG: Password_b.xaml.cs - Constructor - Connection string FOUND (snippet): {connStringSnippet}");
        ...
        private void btnSendResetEmail_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Assumes txtpassword exists in Password_b.xaml with x:Name="txtpassword"
            string newPassword = txtpassword.Text.Trim();
            // Assumes txtconfirm exists in Password_b.xaml with x:Name="txtconfirm"
            ...
            {
                Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] MySQL Error during password update (Password_b): {myEx.ToString()}");
                MessageBox.Show("Error updating password: " + myEx.Message, "Database Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] Generic error during password update (Password_b): {ex.ToString()}");
        ...
        {
            Window6 signInWindow = new Window6(); // Assuming Window6 is your sign-in window
            signInWindow.Show();
